...
function initHeroCanvas() {
    ...
    if (framerBg) {
        ...
        let originalVid = container.querySelector('video');
        if (!originalVid) {
            console.log('[AmpleAI] No video element found. Dynamically creating bulletproof hero video element...');
            originalVid = document.createElement('video');
            originalVid.id = 'ampletechai-hero-video-element';
            originalVid.style.position = 'absolute';
            originalVid.style.top = '0';
            originalVid.style.left = '0';
            originalVid.style.width = '100%';
            originalVid.style.height = '100%';
            originalVid.style.objectFit = 'cover';
            originalVid.style.display = 'block';
            originalVid.style.opacity = '1';
            originalVid.style.zIndex = '1';
            container.appendChild(originalVid);
        }
        ...
        if (originalVid && !originalVid.__intercepted) {
            ...
            originalVid.setAttribute('muted', '');
            originalVid.setAttribute('autoplay', '');
            originalVid.setAttribute('loop', '');
            originalVid.setAttribute('playsinline', '');
            ...
            // Keep source locked in case of replacement
            if (originalVid.getAttribute('src') !== '/hero-bg.mp4') {
                originalVid.setAttribute('src', '/hero-bg.mp4');
            }
...
function initRuntimeInterceptors() {
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        try {
            ...
            console.error('[AmpletechAI] Error in MutationObserver callback:', e);
        } finally {
